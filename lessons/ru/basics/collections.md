...
Ассоциативный массив объявляется с помощью синтаксиса `%{}` и определённых значений или переменных в качестве ключей:
...
Есть несколько способов обратиться к значениям ассоциативного массива:
...
iex> map = %{hello: "world"}
%{hello: "world"}
# Пытаемся обновить ассоциативный массив с новым ключом `:foo`, используя метод `|`
iex> %{map | foo: "baz"}
** (KeyError) key :foo not found in: %{hello: "world"}
    (stdlib) :maps.update(:foo, "baz", %{hello: "world"})
    (stdlib) erl_eval.erl:259: anonymous fn/2 in :erl_eval.expr/5
    (stdlib) lists.erl:1263: :lists.foldl/3
# Используем `Map.put/3` чтобы добавить наш новый ключ и значение
iex> map = Map.put(map, :foo, "baz")
%{foo: "baz", hello: "world"}
# Используем `Map.put/3`, чтобы обновить наш ключ
